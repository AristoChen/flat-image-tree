...
kernel or firmware
    ...
    The value may also name an *image-reference subnode* of this configuration
    node rather than a direct ``/images`` entry; see `Image-reference subnodes`_.
...
image-reference-sub-node-unit-name
    A subnode providing per-configuration load and entry address overrides
    for one image referenced by this configuration.
    See `Image-reference subnodes`_.
...
Image-reference subnodes
------------------------

When a configuration string property (``kernel``, ``firmware``, ``fdt``,
``ramdisk``, ``loadables``) names a value that does not correspond to any
...
The image-reference subnode has the following structure::
...
Example: two configurations share one image but load it at different
addresses::
...
``conf-board-a`` loads the kernel at ``0x80200000`` (override).
``conf-board-b`` loads the kernel at ``0x40200000`` (image-node value).

Mandatory property
~~~~~~~~~~~~~~~~~~

image
    Unit name of the corresponding image node under ``/images``.

Optional properties
~~~~~~~~~~~~~~~~~~~

.. _prop_override_entry:

entry
    Per-configuration override for the entry point address.  When present,
    takes precedence over the ``entry`` property of the referenced image
    node (see :ref:`prop_entry`).  Address size is determined by the
    ``#address-cells`` property of the root node.

.. _prop_override_load:

load
    Per-configuration override for the load address.  When present, takes
    precedence over the ``load`` property of the referenced image node
    (see :ref:`prop_load`).  Address size is determined by the
    ``#address-cells`` property of the root node.
